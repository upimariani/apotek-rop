...
class cTransaksiObat extends CI_Controller
{
	...
	{
		$data = array(
			'transaksi' => $this->db->query("SELECT * FROM `obat_keluar` JOIN pelanggan ON obat_keluar.id_pelanggan=pelanggan.id_pelanggan WHERE id_obat_keluar='" . $id_obat_keluar . "'")->row(),
